...
namespace TiKu.Dal
{
  public class tProblemDal
  {
    public static int Modify(IList<tProblemEntity> objList, string doType,
        string updateFiels, string unupdateFiels)
    {
      string xmlData = PubFun.EntityListToXml(objList, "tProblem", "DataSet",
      ...
      DBHelper.ProcRstInfo rst = DBHelper.ExecuteProc("TiKu", "tProblem_Modify",
    ...
    public static int Delete(string ids)
    {
      ...
      using (XmlWriter xw = XmlWriter.Create(bufXml))
      {
        ...
        PubFun.WriteIDListToXml(xw, ids, "tProblem", "tProblemID");
    ...
    public static tProblemEntity GettProblem(int id)
    {
      tProblemEntity rst = null;
      string strSQL = "SELECT * FROM tProblem WHERE tProblemID=" + id.ToString();
      DataTable dt = DBHelper.QueryToTable("TiKu", strSQL);
      if (dt.Rows.Count > 0)
      {
        rst = new tProblemEntity();
        Trip8H.Common.PubFun.DataRowToObject(dt.Rows[0], rst);
      }
      return rst;
    }

    public static List<tProblemEntity> GettProblemList(int id)
    {
      StringBuilder bufSQL = new StringBuilder();
      List<DbParameter> lstParam = new List<DbParameter>();

      bufSQL.Append("SELECT * FROM tProblem WHERE (1=1) ");

      if (id > 0)
      {
        bufSQL.Append(" AND tProblemID=@id ");
      ...
      List<tProblemEntity> lstRst = PubFun.DataTableToObjects<tProblemEntity>(dtRst);
      return lstRst;
    }
