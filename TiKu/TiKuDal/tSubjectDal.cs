...
namespace TiKu.Dal
{
  public class tSubjectDal
  {
    public static int Modify(IList<tSubjectEntity> objList, string doType,
        string updateFiels, string unupdateFiels)
    {
      string xmlData = PubFun.EntityListToXml(objList, "tSubject", "DataSet",
      ...
      DBHelper.ProcRstInfo rst = DBHelper.ExecuteProc("TiKu", "tSubject_Modify",
    ...
    public static int Delete(string ids)
    {
      ...
      using (XmlWriter xw = XmlWriter.Create(bufXml))
      {
        ...
        PubFun.WriteIDListToXml(xw, ids, "tSubject", "tSubjectID");
    ...
    public static tSubjectEntity GettSubject(int id)
    {
      tSubjectEntity rst = null;
      string strSQL = "SELECT * FROM tSubject WHERE tSubjectID=" + id.ToString();
      DataTable dt = DBHelper.QueryToTable("TiKu", strSQL);
      if (dt.Rows.Count > 0)
      {
        rst = new tSubjectEntity();
        Trip8H.Common.PubFun.DataRowToObject(dt.Rows[0], rst);
      }
      return rst;
    }

    public static List<tSubjectEntity> GettSubjectList(int id)
    {
      StringBuilder bufSQL = new StringBuilder();
      List<DbParameter> lstParam = new List<DbParameter>();

      bufSQL.Append("SELECT * FROM tSubject WHERE (1=1) ");

      if (id > 0)
      {
        bufSQL.Append(" AND tSubjectID=@id ");
      ...
      List<tSubjectEntity> lstRst = PubFun.DataTableToObjects<tSubjectEntity>(dtRst);
      return lstRst;
    }
