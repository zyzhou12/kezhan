...
namespace TiKu.Dal
{
  public class tGradeDal
  {
    public static int Modify(IList<tGradeEntity> objList, string doType,
        string updateFiels, string unupdateFiels)
    {
      string xmlData = PubFun.EntityListToXml(objList, "tGrade", "DataSet",
      ...
      DBHelper.ProcRstInfo rst = DBHelper.ExecuteProc("TiKu", "tGrade_Modify",
    ...
    public static int Delete(string ids)
    {
      ...
      using (XmlWriter xw = XmlWriter.Create(bufXml))
      {
        ...
        PubFun.WriteIDListToXml(xw, ids, "tGrade", "tGradeID");
    ...
    public static tGradeEntity GettGrade(int id)
    {
      tGradeEntity rst = null;
      string strSQL = "SELECT * FROM tGrade WHERE tGradeID=" + id.ToString();
      DataTable dt = DBHelper.QueryToTable("TiKu", strSQL);
      if (dt.Rows.Count > 0)
      {
        rst = new tGradeEntity();
        Trip8H.Common.PubFun.DataRowToObject(dt.Rows[0], rst);
      }
      return rst;
    }

    public static List<tGradeEntity> GettGradeList(int id)
    {
      StringBuilder bufSQL = new StringBuilder();
      List<DbParameter> lstParam = new List<DbParameter>();

      bufSQL.Append("SELECT * FROM tGrade WHERE (1=1) ");

      if (id > 0)
      {
        bufSQL.Append(" AND tGradeID=@id ");
      ...
      List<tGradeEntity> lstRst = PubFun.DataTableToObjects<tGradeEntity>(dtRst);
      return lstRst;
    }
