...
namespace TiKu.Dal
{
  public class tEmailDal
  {
    public static int Modify(IList<tEmailEntity> objList, string doType,
        string updateFiels, string unupdateFiels)
    {
      string xmlData = PubFun.EntityListToXml(objList, "tEmail", "DataSet",
      ...
      DBHelper.ProcRstInfo rst = DBHelper.ExecuteProc("TiKu", "tEmail_Modify",
    ...
    public static int Delete(string ids)
    {
      ...
      using (XmlWriter xw = XmlWriter.Create(bufXml))
      {
        ...
        PubFun.WriteIDListToXml(xw, ids, "tEmail", "tEmailID");
    ...
    public static tEmailEntity GettEmail(int id)
    {
      tEmailEntity rst = null;
      string strSQL = "SELECT * FROM tEmail WHERE tEmailID=" + id.ToString();
      DataTable dt = DBHelper.QueryToTable("TiKu", strSQL);
      if (dt.Rows.Count > 0)
      {
        rst = new tEmailEntity();
        Trip8H.Common.PubFun.DataRowToObject(dt.Rows[0], rst);
      }
      return rst;
    }

    public static List<tEmailEntity> GettEmailList(int id)
    {
      StringBuilder bufSQL = new StringBuilder();
      List<DbParameter> lstParam = new List<DbParameter>();

      bufSQL.Append("SELECT * FROM tEmail WHERE (1=1) ");

      if (id > 0)
      {
        bufSQL.Append(" AND tEmailID=@id ");
      ...
      List<tEmailEntity> lstRst = PubFun.DataTableToObjects<tEmailEntity>(dtRst);
      return lstRst;
    }
