...
namespace TiKu.Dal
{
  public class tHistoryDal
  {
    public static int Modify(IList<tHistoryEntity> objList, string doType,
        string updateFiels, string unupdateFiels)
    {
      string xmlData = PubFun.EntityListToXml(objList, "tHistory", "DataSet",
      ...
      DBHelper.ProcRstInfo rst = DBHelper.ExecuteProc("TiKu", "tHistory_Modify",
    ...
    public static int Delete(string ids)
    {
      ...
      using (XmlWriter xw = XmlWriter.Create(bufXml))
      {
        ...
        PubFun.WriteIDListToXml(xw, ids, "tHistory", "tHistoryID");
    ...
    public static tHistoryEntity GettHistory(int id)
    {
      tHistoryEntity rst = null;
      string strSQL = "SELECT * FROM tHistory WHERE tHistoryID=" + id.ToString();
      DataTable dt = DBHelper.QueryToTable("TiKu", strSQL);
      if (dt.Rows.Count > 0)
      {
        rst = new tHistoryEntity();
        Trip8H.Common.PubFun.DataRowToObject(dt.Rows[0], rst);
      }
      return rst;
    }

    public static List<tHistoryEntity> GettHistoryList(int id)
    {
      StringBuilder bufSQL = new StringBuilder();
      List<DbParameter> lstParam = new List<DbParameter>();

      bufSQL.Append("SELECT * FROM tHistory WHERE (1=1) ");

      if (id > 0)
      {
        bufSQL.Append(" AND tHistoryID=@id ");
      ...
      List<tHistoryEntity> lstRst = PubFun.DataTableToObjects<tHistoryEntity>(dtRst);
      return lstRst;
    }
