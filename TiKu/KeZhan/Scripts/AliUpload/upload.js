...
var uploader = new plupload.Uploader({
    ...
    filters: {
        ...
        { title: "Video files", extensions: "mp4,avi,mov,flv,wmv" }
        ],
	...
	init: {
		    ...
		    plupload.each(files, function (file) {
		      ...
		      // alert($("#hidSumSize").val())
		        if ((parseInt(file.size) + parseInt($("#hidSumSize").val()))/1024 < 20971520) {
		            ...
		            alert("您已超过允许的容量，请删除不再使用的文件后再上传，其中正在使用的文件不可删除");
		        }
			});
		},
		...
		FileUploaded: function(up, file, info) {
            if (info.status == 200)
            {
                ...
                $.ajax({
                    url: "/LoginRequest/UploadFileSuccess",
                    data: { "strkey": g_object_name,"strFileType":$("#dirtype").val(),"strFileName":file.name },
                    dataType: "json",
