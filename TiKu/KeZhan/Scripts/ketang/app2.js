...
methods: {
  ...
  initLogin2(username,role,roomid) {
      this.account = username;
      this.roomID=roomid;
      if(role=="Teacher")
      {
...
if (type === 'TIMTextElem') {
    ...
    if(content.getExt()=="apply_permission_notify"){
        ...
        this.triggerDialog({
            title: '是否接受学生' + msgs.getFromAccount() + '连麦？',
...
startRTC() {
    ...
    if (WebRTC.global.localStream) {
        WebRTC.updateStream({
            ...
            this.showErrorTip(`更新流失败，${JSON.stringify(error)}`);
});
} else {
    WebRTC.startRTC({
        stream: data.stream,
        ...
        this.showErrorTip(`推流失败, ${JSON.stringify(error)}`);
});
}
}, (error) => {
...
closeStudent: function (studentUserId) { 
...
stopStudent: function (studentUserId) { 
    this.ticSdk.sendCustomTextMessage({
        data: "2",
        desc: "test",
        ext: "grant_permission_notify"
 ...
 */
    setColor(color) {
        this.setType(BoardSDK.DRAW_TYPE.LINE);//画曲线
