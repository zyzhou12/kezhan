...
Component({
  ...
  data: {
    ...
    reportData: {
      playUserId: '',
      startTime: 0, // 进房开始时间
      whiteStartTime: 0, // 白名单校验开始时间
      whiteEndTime: 0, // 白名单校验结束时间
      whiteErrorCode: -9999, // 白名单校验失败的错误码
      roomSigStartTime: 0, // roomsig的开始时间
      roomSigEndTime: 0, // roomsig的结束时间
      roomSigErrorCode: -9999, // roomsig失败的错误码
      pushStartTime: 0, // 推流开始时间
      pushSuccTime: 0, // 推流成功时间
      pushErrorCode: -9999, // 推流失败错误码
      playStartTime: 0, // 拉流开始时间
      playSuccTime: 0, // 拉流成功时间
      playErrorCode: -9999, // 拉流失败错误码
      endTime: 0 // 结束时间
    }
  },
  ...
  methods: {
    ...
    initLayout(templateName) {
      ...
      elkReport.reportData('init_layout', {
        extra_info: {
          template: templateName
        }
      });
    ...
    start: function (isNetWorkChange) {
      console.log("isNetWorkChange")
      self = this;
      this.data.hasExitRoom = false;

      elkReport.reportData('init_rtc', {
        extra_info: {
          useCloud: this.data.useCloud,
          originPushURL: this.data.originPushURL,
          isNetWorkChange: isNetWorkChange,
          enableIM: this.data.enableIM,
          pureAudioPushMod: this.data.pureAudioPushMod,
    ...
    pause: function () {
      ...
      elkReport.reportData('pause_rtc');
    },
    ...
    resume: function () {
      ...
      elkReport.reportData('resume_rtc');
    },
    ...
    switchCamera: function () {
      ...
      elkReport.reportData('switch_camera');
    },
    ...
    exitRoom: function () {
      ...
      elkReport.reportData('exit_room');
    },
    ...
    postEvent: function (tag, code, detail) {
      elkReport.reportData('room_event', {
        extra_info: {
          tag: tag,
          code: code,
          data: detail
      ...
      wx.request({
        ...
        success: function (res) {
          ...
          if (res.data["RspHead"]["ErrorCode"] != 0) {
            ...
            elkReport.reportData('request_roomsig', {
              action_result: res.data["RspHead"]["ErrorCode"],
              extra_info: {
                error: res
              }
            });
    ...
    onError: function (e) {
      ...
      self.postErrorEvent(CONSTANT.ROOM.ERROR_CAMERA_MIC_PERMISSION, e.detail.errMsg || '未获取到摄像头、录音功能权限，请删除小程序后重新打开');

      elkReport.reportData('live_pusher_onrror', {
        action_result: e.detail.errCode,
        extra_info: {
          event: e
        }
      });
